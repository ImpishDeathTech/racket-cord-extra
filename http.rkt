...
(define (discord-url . parts)
  (apply ~a "/api/v9" parts #:separator "/"))
...
(define/endpoint (create-message
  ...
  (define all-embeds
    (cond [(embed-builder? single-embed) (cons (send single-embed build) embeds)]
          [(jsexpr? single-embed) (cons single-embed embeds)]
          [else embeds]))
  (unless (null? all-embeds)
    (hash-set! data 'embeds all-embeds))
  (define payload
    (cond [(attachment? attachment) 
           (multipart-payload
          ...
          [(attachment-builder? attachment)
           (let ([attachment* (send attachment build)])
             (multipart-payload
              (field-part "payload_json" (jsexpr->string data) #"application/json")
              (file-part "file"
                         (open-input-bytes (attachment-data attachment*))
...
(define/endpoint (get-guild-emoji _client guild-id emoji-id)
  (make-emoji (get "guilds" guild-id "emojis" emoji-id)))
...
(define/endpoint (get-guild _client guild-id)
  (make-guild (get "guilds" guild-id)))

(define/endpoint (modify-guild _client guild-id data)
  (patch "guilds" guild-id)
  #:data (json-payload (if (guild? data) (guild-data data) data)))
...
(define/endpoint (get-guild-member _client guild-id user-id)
  (make-member (get "guilds" guild-id "members" user-id)))
...
(define/endpoint (add-guild-member _client guild-id user-id data)
  ...
  #:data (json-payload (if (member? data) (member-data data) data)))
...
(define/endpoint (get-current-user _client)
  (make-user (get "users" "@me")))

(define/endpoint (get-user _client user-id)
  (make-user (get "users" user-id)))
